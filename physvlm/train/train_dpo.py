...
from stllm.train.constants import IGNORE_INDEX, X_TOKEN_INDEX, DEFAULT_X_TOKEN, X_INDEX_TOKEN
from stllm.train.stllm_trainer import LLaVADPOTrainer
from stllm.datasets.data_utils import load_jsonl, load_json
import stllm.tasks as tasks
from stllm.common.config import Config
from stllm.datasets.datasets.llavavid_processor import LlavaNextVidProcessor
from stllm.conversation import llavahound_conversation as conversation_lib
conversation_lib.default_conversation = conversation_lib.conv_templates["vicuna_v1"]
from stllm.train.utils import DPODataCollatorWithPadding
    ...
    def tokenize_batch_element(
        ...
        if self.clip_tokenizer is not None:
            if not self.both:
                ...
                clip_input = self.clip_tokenizer(question, return_tensors='pt', padding='max_length', truncation=True)
                batch.update(
                    {
                ...
                clip_input_chosen = self.clip_tokenizer(clip_chosen, return_tensors='pt', padding='max_length', truncation=True)
                clip_input_rejected = self.clip_tokenizer(clip_rejected, return_tensors='pt', padding='max_length', truncation=True)
                batch.update(
                    {
...
def train():
    ...
    if model_cfg.clip_weight:
        clip_tokenizer = CLIPTokenizer.from_pretrained(model_cfg.clip_weight)
