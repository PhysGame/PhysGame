...
from physvlm.models.dpo_model import PreTrainedModelWrapper, create_reference_model
...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # model already loaded, skipped this part.
    ...
    def concatenated_inputs(
        ...
        if 'clip_input_ids' in batch:
            clip_ids_device = [ci.to(device=device) for ci in batch["clip_input_ids"]]
            concatenated_batch["concatenated_clip_ids"] = clip_ids_device + clip_ids_device
            # concatenated_batch["concatenated_clip_mask"] = batch["clip_attention_mask"].repeat(2, 1).to(device=device)
        elif 'clip_chosen_input_ids' in batch:
            concatenated_batch["concatenated_clip_ids"] = torch.cat((batch['clip_chosen_input_ids'],batch['clip_rejected_input_ids']),dim=0)
            # concatenated_batch["concatenated_clip_mask"] = torch.cat((batch['clip_chosen_attention_mask'],batch['clip_rejected_attention_mask']),dim=0)
        repeated_list = [
            batch['images'][0] * 2,
            batch['images'][1] * 2                                
        ]
        batch_pixel_values = []
        batch_image_sizes = []
        for imgs in repeated_list[0]:
            batch_pixel_values.append(imgs['pixel_values'].to(device=device))
            batch_image_sizes.append(imgs['image_sizes'][0].to(device=device))
        concatenated_batch['concatenated_pixel_values'] = batch_pixel_values #torch.cat(batch_video,dim=0).to(device=device)
        concatenated_batch['concatenated_image_sizes'] = batch_image_sizes 
    ...
    def concatenated_forward(
        ...
        all_logits, new_labels = model(
            ...
            clip_ids=concatenated_batch["concatenated_clip_ids"],
            image_sizes=concatenated_batch["concatenated_image_sizes"],
            clip_mask=None,#concatenated_batch["concatenated_clip_mask"],
            use_cache=False,
            dpo_forward=True,
        )
