...
from physvlm.test.video_transforms import (
...
CONV_instructblip_Vicuna0_ = Conversation(
    system="Follow the instructions carefully and explain your answers based on the provided video. ",
    roles=("Human: ", "Assistant: "),
    messages=[],
    instruction=False,
    offset=2,
...
class Chat:
    def __init__(self, model, device='cuda:0'):
        ...
        stop_words_ids = [torch.tensor([835]).to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device),
                          torch.tensor([2277, 29937]).to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device)]  # '###' can be encoded in two different ways.
    ...
    def upload_img(self, image, conv, img_list, text=None):
        if isinstance(image, str):  # is a image path
            raw_image = Image.open(image).convert('RGB')
            image = self.transform([raw_image]).to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device)
            ...
            image = image.to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device)

        image_emb, _, _ = self.model.encode_img(image.unsqueeze(0), text=text)
    ...
    def upload_video(self, video, conv, img_list, num_frame=64, text=None):
        ...
        video_frames = self.transform(raw_frames).to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device) 
        ...
        if self.model.video_input == 'mean':
            ...
            global_embeds = global_embeds.expand((residual_size,-1,-1)).to(f'cuda:{self.model.up_proj.weight.dtype}' if isinstance(self.model.up_proj.weight.dtype, int) else self.model.up_proj.weight.dtype)
    ...
    def get_context_emb(self, conv, img_list):
        ...
        seg_tokens = [
            self.model.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=i == 0).to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device).input_ids
    ...
    def get_context_emb_sim(self, conv, img_list, system=True):
        ...
        seg_tokens = self.model.llama_tokenizer(
                [question], return_tensors="pt", add_special_tokens=0 == 0).to(f'cuda:{self.device}' if isinstance(self.device, int) else self.device)
        ...
        atts_img = torch.ones(img_list[0].size()[:-1], dtype=torch.long).to(f'cuda:{mixed_embs.device}' if isinstance(mixed_embs.device, int) else mixed_embs.device)
