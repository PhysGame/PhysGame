...
video_root = '/group/40033/public_datasets/Video-MME/data'
subtitle_root = '/group/40033/public_datasets/Video-MME/subtitle'
class VideoMME_dataset(EvalDataset):
    def __init__(self, data_dir, num_segments=8, use_subtitles=False):
...
def infer_videomme_stllm(
    ...
    TC, H, W = video.shape
    video = video.reshape(TC//3, 3, H, W).to("npu:0")
    ...
    with torch.no_grad():
        ...
        video_emb, _, _ = encode_model.encode_img(video, question)
    ...
    if data_sample['subtitles']:
        system = "This video's subtitles are listed below: " + data_sample['subtitles'] + ' \n' + system
