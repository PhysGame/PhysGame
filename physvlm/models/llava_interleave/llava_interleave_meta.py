...
class LlavaInterleaveMetaForConditionalGeneration(ABC):
    def encode_image0(self, pixel_value, image_size, clip_ids=None, clip_mask=None):
        ...
        image_feature = self.multi_modal_projector.linear_1(selected_image_feature)
        image_feature = self.multi_modal_projector.act(image_feature)
        image_feature = self.multi_modal_projector.linear_2(image_feature)
    ...
    def encode_image2(self, pixel_value, image_size, clip_ids=None, clip_mask=None):
        batch_size, num_patches, num_channels, height, width = pixel_value.shape
        reshaped_pixel_values = pixel_value.view(batch_size * num_patches, num_channels, height, width)
        image_features = self.vision_tower(reshaped_pixel_values, output_hidden_states=True)
        selected_image_feature = image_features.hidden_states[self.vision_feature_layer]
        ...
        image_feature = self.multi_modal_projector(selected_image_feature,
                                                    clip_ids, clip_mask, selected_image_feature.size(0))
        image_feature = einops.rearrange(image_feature, 'B (T L) D -> (B T) L D', T=selected_image_feature.size(0))
    ...
    def encode_video(self, pixel_value, image_size, clip_ids=None, clip_mask=None, multi_image=False, output_visual_attention=False): 
        ...
        if self.pooling == 'avp':
            ...
            image_features = einops.rearrange(image_features, '(B T) L D -> B (T L) D', T=num_frames)
        elif self.pooling == 'pllava' or self.pooling == 'pllava_self':
            image_features = self.multi_modal_projector(selected_image_feature,
                                                    batch_size=1, num_frames=num_frames)
        elif self.pooling == 'clipST_3d':
            after = (self.pooling == 'clipST_3d')
            image_features = self.multi_modal_projector(selected_image_feature,
                                                    clip_ids, clip_mask, num_frames, after, video=(not multi_image),output_visual_attention=output_visual_attention)
        return image_features

    def _merge_input_ids_with_image_features222(self, image_features, inputs_embeds, input_ids, attention_mask, labels):
        batch_size = input_ids.shape[0]
        sequence_length = torch.tensor([sum(att) for att in attention_mask]).to(inputs_embeds.device)
        ...
        max_embed_dim = (num_special_image_tokens * (num_image_patches - 1)).max() + input_ids.shape[1]
        max_embed_dim_final = (num_special_image_tokens * (num_image_patches - 1) + sequence_length).max()
        ...
        if labels is None:
            ...
            final_labels = final_labels[:, :max_embed_dim_final]
        final_embedding = final_embedding[:, :max_embed_dim_final]
        final_attention_mask = final_attention_mask[:, :max_embed_dim_final]
        position_ids = position_ids[:, :max_embed_dim_final]
        return final_embedding, final_attention_mask, final_labels, position_ids

    def _merge_input_ids_with_image_features22(self, image_features, inputs_embeds, input_ids, attention_mask, labels):
