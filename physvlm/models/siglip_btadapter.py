...
from transformers import SiglipModel, SiglipConfig, SiglipVisionConfig
from transformers.models.siglip.modeling_siglip import SiglipAttention, SiglipMLP, \
    SiglipVisionTransformer, SiglipPreTrainedModel
from transformers.modeling_outputs import BaseModelOutputWithPooling
from mmengine.model import constant_init
from mmengine.runner.checkpoint import _load_checkpoint

class SiglipVisionModel_BTAdapter(SiglipPreTrainedModel):
    ...
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.vision_model = SiglipVisionTransformer_BTAdapter(config)
...
class SiglipVisionTransformer_BTAdapter(nn.Module):
    def __init__(self, configuration: SiglipVisionConfig): 
        ...
        siglip_vision_model = SiglipVisionTransformer(configuration)

        self.encoder = siglip_vision_model.encoder
        self.embeddings = siglip_vision_model.embeddings
        self.pre_layernorm = None
        self.post_layernorm = siglip_vision_model.post_layernorm

        self.btadapter_S_layers = nn.ModuleList([SiglipLayer_Spatial(configuration, dpr[i]) for i in range(self.depth)])
        self.btadapter_T_layers = nn.ModuleList([SiglipLayer_AttnTime(configuration, dpr[i]) for i in range(self.depth)])
...
class SiglipLayer_Spatial(nn.Module):
    def __init__(self, config: SiglipConfig, layer_num=0.1):
        super().__init__()
...
class SiglipLayer_AttnTime(nn.Module):
    def __init__(self, config: SiglipConfig, T, layer_num=0.1):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = SiglipAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim)
