...
def pad_sequences_1d(sequences, dtype=torch.long, device=torch.device("cpu"), fixed_length=None):
    ...
    if isinstance(sequences[0], list):
        if "torch" in str(dtype):
            sequences = [torch.tensor(s, dtype=dtype, device=f'gpu:{device}' if isinstance(device, int) else device) for s in sequences]
        else:
    ...
    if isinstance(sequences[0], torch.Tensor):
        ...
        padded_seqs = torch.zeros((len(sequences), max_length) + extra_dims, dtype=dtype, device=f'gpu:{device}' if isinstance(device, int) else device)
        mask = torch.zeros((len(sequences), max_length), dtype=torch.float32, device=f'gpu:{device}' if isinstance(device, int) else device)
    else:  # np
...
def read_frames_rawframes(
    ...
    fps = 5
    offset=0 if max_num_frames==-1 else 1

    given_frames = False
    if max_num_frames==-1:
        if isinstance(video_path, list):
            all_frames = video_path
            given_frames = True
        else:
            all_frames = os.listdir(video_path)
        all_frames.sort()
        max_num_frames = len(all_frames)
        filename_tmpl=None
    else:
        filename_tmpl="{:0>6}.jpg"
    
    if num_frames == -1 :
        num_frames = max_num_frames if max_num_frames <= 32 else 32
    ...
    for i, frame_idx in enumerate(frame_indices):
        ...
        if given_frames:
            filepath = all_frames[frame_idx]
        elif filename_tmpl:
            filepath = os.path.join(video_path, filename_tmpl.format(frame_idx))
        else:
            filepath = os.path.join(video_path, all_frames[frame_idx])
        try:
            ...
            if filename_tmpl:
                ...
                filepath = os.path.join(video_path, all_frames[frame_idx-1])
            img_bytes = file_client.get(filepath)
        # Get frame with channel order RGB directly.
        cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
        imgs.append(cur_frame)

    frames = [np.transpose(img, (2,0,1)) for img in imgs]
...
if __name__ == "__main__":
    generate_subtitles('/group/40033/public_datasets/video_chatgpt/Test_Videos/v_zxpQHLwZCug.mp4', {})