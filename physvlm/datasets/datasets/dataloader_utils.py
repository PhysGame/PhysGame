...
import random
import torch
from physvlm.datasets.data_utils import move_to_cuda
...
class MetaLoader(object):
    ...
    def build_iter(self):
        ...
        iter_order = torch.Tensor(iter_order).to(torch.device("cuda")).to(f'cuda:{torch.uint8}' if isinstance(torch.uint8, int) else torch.uint8)
...
class PrefetchLoader(object):
    ...
    def preload(self, it):
        ...
        # self.next_input_npu = torch.empty_like(self.next_input,
        #                                        device='cuda')
        # self.next_target_npu = torch.empty_like(self.next_target,
        #                                         device='cuda')
        ...
        with torch.cuda.stream(self.stream):
            ...
            # side stream.
            # self.next_input_npu.copy_(self.next_input, non_blocking=True)
            # self.next_target_npu.copy_(self.next_target, non_blocking=True)
            # self.next_input = self.next_input_npu
            # self.next_target = self.next_target_npu
