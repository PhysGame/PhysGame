...
from physvlm.datasets.datasets.llavavid_processor import LlavaNextViDTextProcessor, LlavaOnevisionViDTextProcessor
...
class InterleaveBaseDataset(Dataset):
    ...
    def process_qa0(self, qa, msg=""):
        ...
        conversation += (
            ...
            qa[0]["q"] + " " + self.begin_signal + self.role[1] + ": "
        )
...
class Interleave_sft_dataset(InterleaveBaseDataset):
    def __init__(self, ann_files, model_cfg, **kwargs):
        ...
        if model_cfg.get('clip_weight', None):
            ...
            if qwen:
                self.clip_length = 196 if model_cfg.get('long_clip', False) else 64
            else:
                self.clip_length = 248 if model_cfg.get('long_clip', False) else 77
            del clip_processor

        self.video_readers = VIDEO_READER_FUNCS
        self.both_clip = model_cfg.get('long_clip', False)

    def __len__(self):
        return self.num_examples

    def transform(self, images, media_type):
        processor = self.gird_processor if media_type=='image' else self.vid_processor
    ...
    def __getitem__(self, index):
        try:
            ...
            # with open("/code/ppllava_qwen/log.txt", "a") as f:
            #     f.write(data_path + '\n')
            index = np.random.randint(0, len(self))
            return self.__getitem__(index)

def old_process(processor, instruction, answer):
    instruction_embed = processor.tokenizer(instruction, return_tensors='pt')
    q_ids, q_att = instruction_embed['input_ids'], instruction_embed['attention_mask']
    answer = answer + processor.tokenizer.eos_token
    answer_embed = processor.tokenizer(answer, return_tensors='pt', add_special_tokens=False)
