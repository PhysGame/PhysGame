...
from physvlm.datasets.datasets.instruction_data import available_corpus, train_transform
from physvlm.datasets.datasets.llavavid_processor import LlavaNextViDTextProcessor
from physvlm.datasets.datasets.llavaovqwen_processor import LlavaOnevisionVidProcessor

import json
from os.path import basename
import numpy as np

from transformers import LlavaNextProcessor, CLIPModel, CLIPProcessor, AutoProcessor
...
class LLaVAITVidTrainDataset(ITImgTrainDataset):
    ...
    def __init__(
        ...
        del self.transform
        if no_grid:
            self.processor = AutoProcessor.from_pretrained(llama_model)
            # self.processor = LlavaOnevisionVidProcessor.from_pretrained(llama_model)
        else:
    ...
    def __getitem__(self, index):
        try:
            ...
            q_len = sum(q_att[0])
            a_len = a_ids.size(1)
            #input_ids = torch.cat([q_ids[:,:q_len], a_ids, q_ids[:,q_len+a_len:]], dim=1)
            #input_att = torch.cat([q_att[:,:q_len], a_att, q_att[:,q_len+a_len:]], dim=1)
            ...
            # print(return_dict["pixel_values"].shape)
            return return_dict
...
class LLaVAITVidTrainDatasetQwen(ITImgTrainDataset):
    media_type = "video"
...
if __name__ == "__main__":
    dataset = LLaVAITVidTrainDataset(available_corpus['caption_videochatgpt_full_flat'], transform=train_transform, system="", both_clip=True,
                                video_reader_type='rawframe', num_frames=16, clip_model='/group/40079/ruyangliu/weight/CLIP-L334', max_length=248,
                                llama_model="/group/40079/ruyangliu/weight/llava-7b-v1.6")

    #dataset = LLaVAITVidTrainDataset(available_corpus['reasoning_next_qa'], transform=train_transform, system="", both_clip=True,
    #                            num_frames=16, clip_model='/group/40079/ruyangliu/weight/CLIP-L334', max_length=248,
    #                            llama_model="/group/40079/ruyangliu/weight/llava-7b-v1.6")
    #dataset = LLaVAITVidTrainDataset(available_corpus['classification_k400'], transform=train_transform, system="",
    #                            num_frames=16, llama_model="/group/40034/palchenli/llama_weight/llava-7b-v1.6")
    
    #dataset = PTVidTrainDataset(available_corpus['webvid'], transform=train_transform, num_frames=16, use_instruction=True,
    #                            model_config={'clip_model':"/group/40034/palchenli/llama_weight/CLIP-L334"})
    #q = []
    a = []
    for example in tqdm(dataset):
        pass
        #q.append(int(example['clip_mask'].sum()))
        a.append(int(example['clip_answer_mask'].sum()))
#
    #b = 1