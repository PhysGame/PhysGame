...
from physvlm.tasks.base_task import BaseTask
from physvlm.datasets.datasets.instruction_data import available_corpus, train_transform
from physvlm.datasets.datasets.image_video_itdatasets import ITImgTrainDataset, ITVidTrainDataset,\
    LLaVAITVidTrainDataset, LLaVAITVidTrainDatasetQwen, PTVidTrainDataset, PTImgTrainDataset
from physvlm.datasets.datasets.interleave_sft_dataset import Interleave_sft_dataset
...
class VideoTextItTask(ImageTextPretrainTask):
    ...
    def build_datasets(self, cfg):
        ...
        llava = ('llava_vid' in cfg.model_cfg.get('arch')) or ('llava_vid_qwen' in cfg.model_cfg.get('arch'))
        for name in datasets_config:
            dataset_config = datasets_config[name]
            dataset_info = available_corpus[name]
            if llava:
                dataset_cls = LLaVAITVidTrainDatasetQwen
                dataset_config['llama_model'] = cfg.model_cfg.get('llama_model')
                dataset_config['no_grid'] = (cfg.model_cfg.get('arch') == 'llava_vid_nogrid') or (cfg.model_cfg.get('arch') == 'llava_vid_qwen_nogrid')
